...
describe("calculateOffset function", () => {
    test("calculates total offset correctly", () => {
      ...
      const totalOffset = calculateOffset(mockOffsets);
...
//   test("creates topic correctly", async () => {
...
//     render(<ClusterTopics topics={mockedTopics} clusterInfo={mockedClusterInfo} clusterid={1} />);
...
//     fireEvent.change(screen.getByLabelText("Number of Partitions"), { target: { value: 3 } });
